...
# Use either the encrypted keystore pair or the relayer private key.
...
For Vercel, keep the repository root as the project root so workspace dependencies resolve. Use `npm ci` for installation and `npm run build --workspace @admon/web` for the build command. Do not set a start command; Vercel runs the Next.js routes as serverless functions. For a self-hosted Node process, use `npm run start --workspace @admon/web -- -p 3000` instead.
